...
function Edit() {
  ...
  const { id } = useParams();
  const crud = useSelector((state) => state.SingleAxiosCrud.data);
  const isLoading = useSelector((state) => state.AxiosCrud.loading);
  const url = useSelector((state) => state.AxiosCrud.url);
  const isFileLoading = useSelector((state) => state.AxiosCrud.fileLoading);
